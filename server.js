...
KILN INTERIOR — EVERYTHING INSIDE IS FRAGILE:
This applies any time someone is loading, unloading, working inside, moving, or maintaining a kiln — not just when replacing elements.
- SOFT BRICK (IFB — Insulating Firebrick): The walls, floor, and lid of most electric kilns are made of soft firebrick. It looks solid but it is extremely fragile — much softer than regular brick. It cracks, chips, and crumbles very easily. A piece that bumps a kiln wall, a shelf that scrapes the side, a tool dropped inside, or even pressing too hard on the wall can crack or gouge the soft brick. Once damaged, soft brick is difficult and expensive to repair and can compromise the kiln's ability to hold temperature. ALWAYS warn potters to be gentle and deliberate inside the kiln at all times.
- LOADING AND UNLOADING: Move slowly and carefully. Never drag kiln shelves across the floor bricks — lift them. Don't let pieces tip or fall against the walls. Don't rest tools against the kiln interior. Bumping the wall with a shelf, a pot, or your arm is one of the most common ways soft brick gets damaged.
- THERMOCOUPLE: The thermocouple is the temperature sensor that sticks into the kiln chamber — it's a thin metal rod, usually in the upper section of the kiln wall. It is fragile and can be bent or broken if bumped during loading or unloading. A damaged thermocouple gives inaccurate temperature readings and can cause under- or over-firing. Be aware of where it is and never let a shelf, pot, or prop touch it.
- ELEMENTS: The heating elements sit in grooves cut into the soft brick walls. They are also fragile — never touch them with tools, never let a shelf or pot rest against them, and never force anything past them. L&L kilns use ceramic element holders instead of grooves — these are breakable too.
- MOVING A KILN: Kilns are not meant to be moved frequently. If a kiln must be moved, the soft brick interior can crack from vibration and impact. If possible, remove all shelves and props first. Move slowly, avoid bumping or dropping. Even a short move across a floor can cause cracks if the kiln is jolted. After any move, inspect the interior carefully before firing.
- GENERAL RULE: Soft brick is softer than chalk — it is shockingly fragile. To illustrate: fingernails alone can gouge deep marks into it. A brand new kiln can end up with nail marks in the walls just from loading and unloading pieces normally. Potters are often shocked the first time they realize this. Make sure beginners know before they ever open a kiln.
...
app.patch('/api/pieces/:id/public', auth, (req, res) => {
  try {
    const piece = db.prepare('SELECT user_id, status FROM pieces WHERE id=?').get(req.params.id);
    if (!piece) return res.status(404).json({ error: 'Piece not found' });
...
app.get('/api/gallery', (req, res) => {
  try {
    ...
    const pieces = db.prepare(`
      ...
      LEFT JOIN clay_bodies cb ON p.clay_body_id = cb.id
      WHERE p.is_public = 1 AND REPLACE(LOWER(TRIM(p.status)), ' ', '-') IN ('glaze-fired', 'done', 'complete', 'sold')
      ORDER BY p.updated_at DESC
      LIMIT ? OFFSET ?
    `).all(limit, offset);

    const total = db.prepare("SELECT COUNT(*) as c FROM pieces WHERE is_public=1 AND REPLACE(LOWER(TRIM(status)), ' ', '-') IN ('glaze-fired', 'done', 'complete', 'sold')").get().c;
