...
function showMemberGroup(filter) {
  ...
  let rows = filtered.map(x => '<tr style="border-bottom:1px solid var(--border)"><td style="padding:8px">' + esc(x.display_name||'—') + '</td><td style="padding:8px;font-size:0.8rem">' + esc(x.email) + '</td><td style="padding:8px">' + (x.stripe_subscription_id ? '💳 Paid' : (x.tier==='free' ? '🆓 Free' : '🎁 Gifted')) + '</td><td style="padding:8px;font-size:0.8rem">' + (x.created_at?new Date(x.created_at).toLocaleDateString():'') + '</td></tr>').join('');
