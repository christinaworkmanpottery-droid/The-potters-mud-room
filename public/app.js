...
function showMemberGroup(filter) {
  ...
  let filtered = m;
  if (filter === 'paid') filtered = m.filter(x => (x.stripe_subscription_id && x.stripe_subscription_id !== '') || ['month','year'].includes(x.billing_period));
  else if (filter === 'gifted') filtered = m.filter(x => (x.tier==='starter'||['basic','mid','top'].includes(x.tier)) && (!x.stripe_subscription_id || x.stripe_subscription_id === '') && !['month','year'].includes(x.billing_period));
  ...
  let rows = filtered.map(x => '<tr style="border-bottom:1px solid var(--border)"><td style="padding:8px">' + esc(x.display_name||'—') + '</td><td style="padding:8px;font-size:0.8rem">' + esc(x.email) + '</td><td style="padding:8px">' + ((x.stripe_subscription_id && x.stripe_subscription_id !== '') || ['month','year'].includes(x.billing_period) ? '💳 Paid' : (x.tier==='free' ? '🆓 Free' : '🎁 Gifted')) + '</td><td style="padding:8px;font-size:0.8rem">' + (x.created_at?new Date(x.created_at).toLocaleDateString():'') + '</td></tr>').join('');
